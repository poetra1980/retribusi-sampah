...
const COLUMNS = [
  ...
  { key: 'parent', label: 'Induk', render: (r) => r.parent?.name || '-' },
  { key: 'status', label: 'Status', render: (r) => <StatusBadge status={r.status} /> }
];

const INIT_FORM = { code: '', name: '', level: 'kelurahan', status: 'active' };
...
export default function Regions() {
  ...
  const fetchData = useCallback(async (cursor) => {
    ...
    try {
      ...
      const { data: res } = await regionApi.list(params);
      setData(res.data || []);
      setMeta(res.meta);
    } catch {
      setAlert({ type: 'error', message: 'Gagal memuat data wilayah' });
    } finally {
      setLoading(false);
    }
  }, [search]);
  ...
  const openCreate = () => setModal({ open: true, mode: 'create', form: INIT_FORM });

  const openEdit = (row) => {
    setModal({ open: true, mode: 'edit', form: { code: row.code, name: row.name, level: row.level, parentId: row.parentId, status: row.status } });
  };
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (modal.mode === 'create') {
        ...
        setAlert({ type: 'success', message: 'Wilayah berhasil dibuat' });
      } else {
        await regionApi.update(data.find((d) => d.code === modal.form.code)?.id, payload);
        setAlert({ type: 'success', message: 'Wilayah berhasil diperbarui' });
      }
  ...
  return (
    <div>
      <h2 className="page-title">Master Wilayah</h2>

      <AlertMessage type={alert.type} message={alert.message} onClose={() => setAlert({ type: '', message: '' })} />

      <div className="toolbar">
        <SearchInput value={search} onChange={setSearch} placeholder="Cari wilayah..." />
        <button className="btn btn-primary" onClick={openCreate}>Tambah Wilayah</button>
      </div>
      ...
      <Modal isOpen={modal.open} onClose={() => setModal({ ...modal, open: false })} title={modal.mode === 'create' ? 'Tambah Wilayah' : 'Edit Wilayah'}>
        <form onSubmit={handleSubmit}>
          ...
          <div className="form-group">
            <label>Level</label>
            <select className="form-control" value={modal.form.level} onChange={(e) => setModal({ ...modal, form: { ...modal.form, level: e.target.value } })}>
